...
# kruskal.test(bkkca ~ Time, mrna_rep_winxiqr[mrna_rep_winxiqr$Source == "Brian", ])
# ,
#             Kruskal = kruskal.test(Count ~ Time, .data)
...
for (i in 1:nrow(stats)){
  ...
  temp <- mrna_rep_winxiqr %>% filter(Source == t_Source & mRNA == t_mRNA & !is.na(Count))
  
  stats[i, "Main"] <- kruskal.test(Count ~ Time, temp)$p.value
  stats[i, "BvsC"] <- wilcox.test(Count ~ Time, temp[temp$Time != "Delayed",])$p.value
  stats[i, "CvsD"] <- wilcox.test(Count ~ Time, temp[temp$Time != "Baseline",])$p.value
}
...
mrna_rep_medians <- mrna_rep_medians %>% 
  ...
  ungroup() %>% 
  mutate(updown1 = ifelse(Baseline < Compensated, "up", "down"), 
         updown2 = ifelse(Compensated < Delayed, "up", "down"))


# combine stats and prepared plotting df####
mrna_rep_medians <- full_join(mrna_rep_medians, stats) %>% 
  mutate(color1 = ifelse(Main < 0.05 & BvsC < 0.05, updown1, "none"),
         color2 = ifelse(Main < 0.05 & CvsD < 0.05, updown2, "none")) %>% 
...
ggplot(mrna_rep_medians[, ])+
  # geom_point(aes(id, MedianCount))+
  # geom_line(aes(id, MedianCount))+
  # geom_rect(aes(xmin = 1, xmax = 2, 
  #               ymin = Baseline, ymax = Compensated,
  #               fill = updown1))+
  # geom_rect(aes(xmin = 2, xmax = 3, 
  #               ymin = Compensated, ymax = Delayed,
  #               fill = updown2))+
