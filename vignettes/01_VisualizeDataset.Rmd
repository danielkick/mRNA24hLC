...
rownames(M_winxiqr) <- paste(M_winxiqr$Condition, M_winxiqr$Experiment, M_winxiqr$Cell, sep = "-")
```
...
mrna_bd <-select(mrna, 
  ...
  filter(Pharm != "TTX" & Pharm != "AP") %>%     #FIXME dropped for stg meeting presentation
  filter(!(Pharm == "None" & Time == "Delayed")) #FIXME dropped for stg meeting presentation
...
mrna_bd$uid <- as.character(seq(1, nrow(mrna_bd)))
mrna_bd <- mrna_bd %>% 
  select(-Experiment, -Cell) %>% 
  unite(Group, Source, Pharm, Time)

mrna_bd <- mrna_bd[, c("Group", "uid",
                       names(mrna_bd)[!(names(mrna_bd) %in% c("Group", "uid"))])
                   ]


# Remove Outliers 
mrna_bd_winxiqr_list <- map(unique(mrna_bd$Group), function(group){
  temp <- mrna_bd[mrna_bd$Group == group, ]
  dvs <- names(mrna_bd)[!(names(mrna_bd) %in% c("Group", "uid"))]
...
mrna_bd_winxiqr <- imputeMissings::impute(mrna_bd_winxiqr, method = "median/mode")
```
...
vis_pca_3D(input.df = mrna_bd_winxiqr[, names(mrna_bd_winxiqr)[!(names(mrna_bd_winxiqr) %in% c("Group", "uid"))]],
           color.by = mrna_bd_winxiqr[, "Group"],
           turn.x.times = 1,
           use.colors = RColorBrewer::brewer.pal(8, "Paired")[-3]
)



mrna_bd_annotations <- mrna_bd_winxiqr %>% 
