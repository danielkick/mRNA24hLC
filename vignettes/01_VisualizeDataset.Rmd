...
# TODO How do we best handle duplicate obs? median? 
...
epsp %>% 
  group_by(Experiment, Cell, Type, Key) %>% 
  mutate_at(c("Cor", "Min.V", "Max.Amplitude", "AUC"), median, na.rm = T) %>% 
  select(-FileName, -Sweep) %>% 
    distinct()
##

# TODO figure out how Brian labeled cells. 
# mrna[mrna$Source != "Brian", "Cell"]
# mrna[mrna$Source == "Brian", "Cell"] %>% distinct() %>% View
# 
# mrna$Source %>% unique()
...
tevc_cols <- c("ig")
epsp_cols <- c("Cor", "Min.V_Obs", "Min.V_Sim", "Max.Amplitude_Obs", "Max.Amplitude_Sim", "AUC_Obs", "AUC_Sim")
...
M_ft <-flextable(fm_overview_table,
                 col_keys = c("Family", "dv", "p", "fdr", "stars"))
...
# lm(Ihtk.0 ~ Condition, data = temp)
# 
# fm <- nlme::lme(Ihtk.0 ~ Condition, random = ~1|Experiment, data = temp[!is.na(temp$Ihtk.0),])
# 
# nlme::anova.lme(fm)
# # cell nested in experiment, experiment rand
# 
# temp
# 
# # the only occasion to nest (1 | Experiment/Cell) is if there are multiple sweeps
# fm2 <- lme4::lmer(Ihtk.0 ~ Condition + (1 | Experiment), data = temp[!is.na(temp$Ihtk.0),])
# 
# summary(fm2)
# 
# broom::tidy(fm2)
# 
# broom.mixed::tidy(fm2)
# 
...
# fm0 <- lme(Rin_n050 ~ 1, random = ~1|Mouse, data = M[!is.na(M$Rin_n050), ], method = "ML")
# fm1 <- lme(Rin_n050 ~ Condition, random = ~1|Mouse, data = M[!is.na(M$Rin_n050), ], method = "ML")
# fm2 <- lme(Rin_n050 ~ DPI, random = ~1|Mouse, data = M[!is.na(M$Rin_n050), ], method = "ML")
# fm3 <- lme(Rin_n050 ~ CellType, random = ~1|Mouse, data = M[!is.na(M$Rin_n050), ], method = "ML")
# fm4 <- lme(Rin_n050 ~ Condition*DPI, random = ~1|Mouse, data = M[!is.na(M$Rin_n050), ], method = "ML")
# fm5 <- lme(Rin_n050 ~ Condition*CellType, random = ~1|Mouse, data = M[!is.na(M$Rin_n050), ], method = "ML")
# fm6 <- lme(Rin_n050 ~ DPI*CellType, random = ~1|Mouse, data = M[!is.na(M$Rin_n050), ], method = "ML")
# fm7 <- lme(Rin_n050 ~ Condition*DPI*CellType, random = ~1|Mouse, data = M[!is.na(M$Rin_n050), ], method = "ML")
# 
# aictab(cand.set = list(fm0, fm1, fm2, fm3, fm4, fm5, fm6, fm7))
# 
# 
# anova.lme(fm5)
# 
# lsmeans(fm, pairwise~Condition*CellType, adjust = "tukey")
# 
# 
# 
# fm <- lme(Rin_n050 ~ Condition*DPI*CellType, random = ~1|Mouse, data = M[!is.na(M$Rin_n050), ], method = "REML")
# anova.lme(fm)
# 
# lsmeans(fm, pairwise~Condition, adjust = "tukey")
