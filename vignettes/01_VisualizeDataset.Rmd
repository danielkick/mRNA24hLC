...
plt <- {pca_summary_descriptions[[1]]$scree | pca_summary_descriptions[[2]]$scree | pca_summary_descriptions[[3]]$scree}/
...
set.seed(16069)
temp.umap = umap(M_winxiqr[, names(M_winxiqr)[!(names(M_winxiqr) %in% c(
  'Condition', 'Cell', 'Experiment', 'cc.3_4', 'cc.3_5', 'ig.3_4', 'ig.3_5'))]]
  )
...
temp_list_hsd <- map(seq_along(temp_list[[4]]), function(i){
  if (sum(!is.na(unique(temp_list[[4]][[i]]$groups)))){
    print(i)
    temp_list[[4]][[i]] %>% 
...
for (i in seq_along(membrane_dvs_plt)){ # <--------------------------------------- !
  ...
  if (temp_annotation$fdr <= 0.05){
    ...
    # changed so that multiple insignificnat values e.g. i==7, steps_up 0,0,1 won't
    # cause issues
    df$y.position <- start_y + step_size*steps_up
