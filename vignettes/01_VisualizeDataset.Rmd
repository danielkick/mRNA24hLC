...
```{r}
#FIXME
# vis_pca_3D(input.df = mrna_bd_winxiqr_interp[, names(mrna_bd_winxiqr_interp)[!(names(mrna_bd_winxiqr_interp) %in% c("Group", "uid"))]],
#            color.by = mrna_bd_winxiqr_interp[, "Group"],
#            turn.x.times = 1,
#            use.colors = RColorBrewer::brewer.pal(9, "Paired")
# )

# vis_pca_3D(input.df = select(mrna_b_winxiqr_interp, -Group, -uid),
...
# 
# mrna_bd_annotations <- mrna_bd_winxiqr_interp %>% 
#   separate(Group, c("Source", "Pharm", "Time")) %>% 
#   select(Source, Pharm, Time)
# 
# vis_pca_3D(input.df = mrna_bd_winxiqr_interp[, names(mrna_bd_winxiqr_interp)[!(names(mrna_bd_winxiqr_interp) %in% c("Group", "uid"))]],
#            color.by = mrna_bd_annotations[, "Source"],
#            turn.x.times = 1,
#            use.colors = RColorBrewer::brewer.pal(3, "Set1")
# )
# 
# vis_pca_3D(input.df = mrna_bd_winxiqr_interp[, names(mrna_bd_winxiqr_interp)[!(names(mrna_bd_winxiqr_interp) %in% c("Group", "uid"))]],
#            color.by = mrna_bd_annotations[, "Pharm"],
#            turn.x.times = 1,
#            use.colors = RColorBrewer::brewer.pal(3, "Set1")
# )
# 
# vis_pca_3D(input.df = mrna_bd_winxiqr[, names(mrna_bd_winxiqr)[!(names(mrna_bd_winxiqr) %in% c("Group", "uid"))]],
#            color.by = mrna_bd_annotations[, "Time"],
#            turn.x.times = 1,
#            use.colors = RColorBrewer::brewer.pal(3, "Set1")
# )
...
# ggplot(temp, aes(V1, V2, color = col_by_df$Blocker))+
#   geom_point()+
#   geom_density2d()
# 
# ggplot(temp, aes(V1, V2, color = col_by_df$Time))+
#   geom_point()
# 
# 
# ggplot(temp, aes(V1, V2, 
#                  group = mrna_b_winxiqr_interp$Group))+
#   geom_point(aes(color = mrna_b_winxiqr_interp$Group))+
#   stat_density_2d(aes(#color = mrna_b_winxiqr_interp$Group, 
#                       fill = mrna_b_winxiqr_interp$Group), geom = "polygon", alpha = 0.03)
# 
# 
# ggplot(temp, aes(V1, V2, 
#                  group = col_by_df$Blocker))+
#   stat_density_2d(aes(#color = mrna_b_winxiqr_interp$Group, 
#                       fill = col_by_df$Blocker), geom = "polygon", alpha = 0.1)+
#   geom_point(aes(color = col_by_df$Blocker))
...
df <- temp
df$Blocker <- col_by_df$Blocker
find_hull <- function(df) df[chull(df$V1, df$V2), ]
hulls <- plyr::ddply(df, "Blocker", find_hull)

ggplot(df, aes(V1, V2, color = Blocker, fill = Blocker))+
  geom_polygon(data = hulls, alpha = 0.1) +
  geom_point()


df <- temp
df$Blocker <- col_by_df$Time
find_hull <- function(df) df[chull(df$V1, df$V2), ]
hulls <- plyr::ddply(df, "Blocker", find_hull)
...
df <- temp
df$Blocker <- mrna_b_winxiqr_interp$Group
find_hull <- function(df) df[chull(df$V1, df$V2), ]
hulls <- plyr::ddply(df, "Blocker", find_hull)
...
```{r eval=FALSE}
set.seed(654684)
cluster <- pvclust(t(mrna_bd_winxiqr[, names(mrna_bd_winxiqr)[!(names(mrna_bd_winxiqr) %in% c("Group", "uid"))]]),
...
# library(corrr)
# 
# cor_bd <- mrna_bd_winxiqr %>% select(-Group, -uid) %>% correlate()



library(corrplot)
corrplot(cor(select(mrna_bd_winxiqr, -Group, -uid)), 
         p.mat = cor.mtest(select(mrna_bd_winxiqr, -Group, -uid), conf.level = .95)$p,
         sig.level = .01,
         insig = "blank",
         method = "color", 
         type = "upper",
...
for (group in unique(mrna_bd_winxiqr$Group)){
  tiff(file=here("officer_output", paste0("ContrastSource_", group, "_cor.tiff")),
       width=7.96, height=6.68, units="in", res=100)
  
  corrplot(cor(select(mrna_bd_winxiqr[mrna_bd_winxiqr$Group == group, ], -Group, -uid)), 
           p.mat = cor.mtest(select(mrna_bd_winxiqr, -Group, -uid), conf.level = .95)$p,
           sig.level = .01,
           insig = "blank",
           method = "color", 
...
ecdfs
```{r eval=FALSE}
cor_list_df <- map(unique(mrna_bd_winxiqr$Group), function(group){
  temp <- cor(select(mrna_bd_winxiqr[mrna_bd_winxiqr$Group == group, ], -Group, -uid)) %>% as.data.frame()
...
map(list(
    ## ----
  list(
  cor_list_df[cor_list_df$Group %in% c(
    "Brian_None_Baseline", 
    "Brian_TEA_Compensated"
    ), ],
  c("Brian_None_Baseline", 
    "Brian_TEA_Compensated"
    #"Brian_TEA_Delayed", 
    #"Daniel_None_Baseline", 
    #"Daniel_TEA_Compensated", 
    #"Daniel_TEA_Delayed"
    ),
  c( "#a6cee3", 
     "#1f78b4"
