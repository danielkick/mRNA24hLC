...
# ggplot()+geom_histogram(aes(x = null_jaccard))
...
```{r}
#TODO
mrna_cols <- c("bkkca", "cav1", "cav2", 
               "inx1", "inx2", "inx3", #"inx4", "inx5", #<- inx5 is likely not expressed
               "nav", 
               "shab", "shaker", "shal", "shaw1", "shaw2")

memb_cols <- c("vrest", "r11", "r1", "Ihtk.0", "Ihtk.Slope", "Ia.0", "Ia.Slope")
...
(o_mrna$group_compare_tile+
  ...
  (o_mrna$heatmap_z + theme(legend.position = "below")) + 
...
(o_mem$group_compare_tile+
  ...
  (o_mem$heatmap_z + theme(legend.position = "below")) + 
...
(o_exc$group_compare_tile+
  ...
  (o_exc$heatmap_z + theme(legend.position = "below")) + 
  patchwork::plot_layout(heights = c(.2, 1))
...
plt <- do.call(rbind, temp_list) %>% 
  ...
  # mutate(CorFill = case_when(
  #   cor < cor_br[2] & cor >= cor_br[1] ~ cor_br[1],
  #   cor < cor_br[3] & cor >= cor_br[2] ~ cor_br[2],
  #   cor < cor_br[4] & cor >= cor_br[3] ~ cor_br[3],
  #   cor < cor_br[5] & cor >= cor_br[4] ~ cor_br[4],
  #   
  #   
  #                            )) %>% 
...
# Time = "0h"
cor_plt_list <- map(1:3, function(Step){
  
  comparison_list <- list(list(
    c("0h"), c("1h"), c("0h vs 1h")
  ),
  list(
    c("1h"), c("24h"), c("1h vs 24h")
  ),
  list(
    c("0h"), c("24h"), c("0h vs24h")
  ))
  
  
  df = M_winxiqr[M_winxiqr$Condition == comparison_list[[Step]][[1]], 
                 c(mrna_cols, memb_cols, epsp_cols)]
  
  corr <- cor(df, use = "pairwise.complete.obs", method = "spearman")
  # p.mat <- cor_pmat(df, use = "pairwise.complete.obs", method = "spearman")
  
  df2 = M_winxiqr[M_winxiqr$Condition == comparison_list[[Step]][[2]], 
                 c(mrna_cols, memb_cols, epsp_cols)]
  
  corr2 <- cor(df2, use = "pairwise.complete.obs", method = "spearman")
  
  
  cor_df_bin <- corr2 - corr 
  cor_bins <- seq(-1, 1, length.out = 8)
  for (i in 1:(length(cor_bins)-1)){
    # use the more extreme value to shade with
    cor_df_bin[cor_df_bin > cor_bins[i] & cor_df_bin < cor_bins[i+1]] <- cor_bins[c(i, (i+1))[which.max(abs(cor_bins[i:(i+1)]))]]
  ...
  ggcorrplot(
    ...
    labs(subtitle = comparison_list[[Step]][[3]])+
    theme(legend.position = ""#,
          # axis.text.x = element_text(angle = 90)
          )
})
...
# # mrna_cols
# # 
# # c("vrest", "r11", "r1", "Ihtk.0", "Ihtk.Slope", "Ia.0", "Ia.Slope")
# # 
# # epsp_cols
# # 
# # 
...
# 
# # library(GGally)
# ggcorr(M_winxiqr[, mrna_cols],
#            # geom = "circle",
#        hjust = 1,
#        # layout.exp = 1,
#            nbreaks = 5,#9,
#            low = RColorBrewer::brewer.pal(9, "PuOr")[1],
#            mid = RColorBrewer::brewer.pal(9, "PuOr")[5],
#            high = RColorBrewer::brewer.pal(9, "PuOr")[9])
# # 
# # 
# # 
# # ggcorr(M_winxiqr[, mrna_cols], 
# #        geom = "blank", label = F, hjust = 0.75) +
# #   # geom_circle(aes(x0=x, y0=y, r=coefficient, 
# #   #                 color = coefficient > 0, alpha = abs(coefficient) > 0.5))
# #   geom_point(size = 1, aes(color = coefficient > 0, alpha = abs(coefficient) > 0.5)) +
...
# # # #   guides(color = FALSE, alpha = FALSE)
# # # 
# # # ggpairs(M_winxiqr[, mrna_cols])
# # # 
# 
# 
# ## Describe one time point
# ## THen describe chance between them
# 
# 
# 
# 
# cor_list <- map(c("0h", "1h", "24h"), function(Group){
#   df <- M_winxiqr[M_winxiqr$Condition == Group , mrna_cols] #FIXME
#   cor_df <- cor(df, use = "pairwise.complete.obs", method = "spearman")
#   
#   
#   n_bins = 9
#   cor_color_df <- matrix(
#     as.numeric(cut(cor_df, breaks = seq(-1, 1, length.out = n_bins))),
#     nrow = nrow(cor_df),
#     ncol = ncol(cor_df))
#   
#   cor_color_df <- matrix(
#     RColorBrewer::brewer.pal(n_bins, "PuOr")[cor_color_df],
#     nrow = nrow(cor_df),
#     ncol = ncol(cor_df))
#   
#   return(list(cor_df = cor_df,
#               cor_col =cor_color_df))
#   
# })
# 
# i=1
# 
# temp <- 
# data.frame(`0h` = cor_list[[1]]$cor_df[i, ],
#            `1h` = cor_list[[2]]$cor_df[i, ],
#            `24h` = cor_list[[3]]$cor_df[i, ]) %>% 
#   rownames_to_column(var = "rowname")  %>% 
#   group_by(rowname) %>% 
#   mutate(ave_cor = sum(X0h, X1h, X24h)/3) %>%
#   ungroup()
# 
# #### 2d option
# 
...
# temp[2,] %>% 
#   group_by(rowname) %>% 
#   mutate(MAE = (sum(abs(X0h - ave_cor), 
#                     abs(X1h - ave_cor), 
#                     abs(X24h - ave_cor)
#                     )/3
#                 )
#          ) %>% 
#   ggplot()+
#   geom_segment(aes(x = -1, xend = 1, y = -1, yend = 1), color = "black")+
#   geom_point(aes(x = X0h, y = X1h), shape = 1)+
#   # geom_segment(aes(x = X0h, xend = X0h, y = X0h, yend = X1h),
#   #              arrow = arrow(length = unit(0.2,"cm")),
#   #              size = 1,
#   #              color = "orange")+
#   
#   geom_point(aes(x = X1h, y = X24h), shape = 1)+
#   geom_segment(aes(x = X0h, xend = X1h, y = X1h, yend = X24h,
#                    color = MAE), 
#                arrow = arrow(length = unit(0.2,"cm")), 
#                # color = "red",
#                # size = 1)
#                size = 0.1)+ 
#   scale_color_viridis_c(option = "inferno", end = 0.75) 
...
#                 )
#          ) %>% 
#   filter(X0h >= 0.6 | X1h >= 0.6 | X24h >= 0.6) %>% 
#   ggplot()+
#   geom_segment(aes(x = -1, xend = 1, y = -1, yend = 1), color = "black")+
#   geom_point(aes(x = X0h, y = X1h), shape = 1)+
#   # geom_segment(aes(x = X0h, xend = X0h, y = X0h, yend = X1h),
...
#   
#   plt <- plt %>% 
#     add_trace(x = c(temp$X0h[j], temp$ave_cor[j]),
#               y = c(temp$X1h[j], temp$ave_cor[j]),
#               z = c(temp$X24h[j], temp$ave_cor[j]),
#               mode = "lines")
#   
#     
# }
...
#               z = temp$X24h, 
#               color = temp$ave_cor) %>% 
#   add_trace(x = c(0, 1), y = c(0, 0), z = c(0, 0), mode = "lines", color = "") %>% 
#   add_trace(x = c(0, 0), y = c(0, 1), z = c(0, 0), mode = "lines", color = "") %>%   
#   add_trace(x = c(0, 0), y = c(0, 0), z = c(0, 1), mode = "lines", color = "") 
# ####
# 
# 
# 
...
# #   as.data.frame() %>% 
# #   gather("key", "value") %>% 
# #   ggplot(aes(x = value, y = 1, label = key))+
# #   # geom_label()
...
# ## setup ####
# sample_df <- M_winxiqr[M_winxiqr$Condition == "0h" , mrna_cols]
# 
# # sample_df <- sample_df[, 1:2]
# # names(sample_df) <- c(
# #   "obs1",
# #   "obs2"
# # )
# # 
# # sample_df <- sample_df %>% mutate(syn1 = obs1*( 1.3+(obs2*0.01)),
# #                                   syn2 = obs2*(-1.3+(obs1*0.01))
# # )
# 
# p <- ncol(sample_df)
# 
...
# #   for (cl in (rw+1):p){
# #     
# #     plt <- getPlot(p1, rw, cl) +
# #      theme(panel.background = element_rect(fill = colors[color_index], color="white"),
# #            panel.grid.major = element_line(color= colors[color_index]))
# #     
...
# ## replace upper ####
# 
# cor_df <- cor(sample_df, method = "spearman")
# 
# n_bins = 9
# cor_color_df <- matrix(
#   as.numeric(cut(cor_df, breaks = seq(-1, 1, length.out = n_bins))),
...
#     #                   r = abs(round(cor_df[rw, cl], digits = 2))),
#     #               fill = cor_color_df[rw, cl]) +
#     #   geom_text(aes(x = 0, y = 1.5, label = round(cor_df[rw, cl], digits = 2)))+
#     #   theme_void()+
#     #   lims(y = c(-1.5, 1.5), x = c(-1.5, 1.5))+
#     #   coord_fixed()
#     # 
...
# 
# # p1
# ## on unity line no background ####
# for (rw in 1:p){
#   cl <- rw
#   plt <- getPlot(p1, rw, cl)+ theme(panel.background = element_rect(fill = NA), 
...
#     
#     plt <- getPlot(p1, rw, cl)+ 
#       theme(panel.background = element_rect(fill = cor_color_df[rw, cl], color="white"),
#             panel.grid.major = element_line(color= NA), 
...
map(list(
  list(
    cor_list_df[cor_list_df$Condition %in% c("0h", "1h"), ],
    c("0h", "1h"),
    c("#4d4d4d", 
      "#67a9cf"#, 
      #"#1c9099"
  ...
  list(
    cor_list_df[cor_list_df$Condition %in% c("0h", "24h"), ],
    c("0h", "24h"),
    c("#4d4d4d", 
      #"#67a9cf", 
      "#1c9099")
  ...
  list(
    cor_list_df[cor_list_df$Condition %in% c("1h", "24h"), ],
    c("1h", "24h"),
    c(#"#4d4d4d", 
      "#67a9cf",
      "#1c9099")
