...
cor_comp_df %>% 
  ...
  filter(Time == "24h") %>%
  spread(Source, Corr) %>% 
  ggplot(aes(x = Brian, y = Daniel, group = Time, color = Time))+
...
cor_comp_df %>% 
  filter(Time == "24h") %>% 
  spread(Source, Corr) %>% 
  filter(Brian > 0.5 & Daniel > 0.5)


p1 <- 
cor_comp_df %>% 
  spread(Time, Corr) %>% 
  ggplot(aes(x = `0h`, y = `1h`, group = Source, color = Source))+
  geom_abline(intercept = 0, slope = 1)+
  geom_point()+
  geom_smooth(method = "lm", se = F, fullrange = T)+
  coord_fixed()+
...
cor_comp_df %>% 
  spread(Time, Corr) %>% 
  ggplot(aes(x = `0h`, y = `24h`, group = Source, color = Source))+
  geom_abline(intercept = 0, slope = 1)+
  geom_point()+
  geom_smooth(method = "lm", se = F, fullrange = T)+
  coord_fixed()+
...
cor_comp_df %>% 
  spread(Time, Corr) %>% 
  ggplot(aes(x = `1h`, y = `24h`, group = Source, color = Source))+
  geom_abline(intercept = 0, slope = 1)+
  geom_point()+
  geom_smooth(method = "lm", se = F, fullrange = T)+
  coord_fixed()+
...
cor_comp_df %>% 
  ...
  # filter(x == "bkkca") %>% 
  ggplot(aes(y))+
  # geom_rect(aes(xmin = "cav1", xmax= "shaw2", ymin= -1, ymax = 0))+
  geom_hline(yintercept = 0, color = "red", linetype = "dashed")+
  geom_segment(aes(x = y, xend = y, y = Brian, yend = Daniel), size = 2)+
  geom_point(aes(y = Brian), size = 10, color = "darkgray")+
  geom_point(aes(y = Brian), size = 10, shape = 1)+
  geom_point(aes(y = Daniel), size = 10)+
  geom_text(aes(y = Brian, label = round(Brian, digits = 2)),  
            color = "white", fontface = "bold", size = 3)+
  geom_text(aes(y = Daniel, label = round(Daniel, digits = 2)), 
            color = "white", fontface = "bold", size = 3)+
  coord_cartesian(ylim = c(-1, 1))+
  facet_grid(x ~ .)+
...
cor_comp_df %>% 
  ...
  geom_hline(yintercept = 0, color = "red", linetype = "dashed")+
  geom_segment(aes(x = y, xend = y, y = Brian, yend = Daniel), size = 2)+
  geom_point(aes(y = Brian), size = 2, color = "darkgray")+
  geom_point(aes(y = Brian), size = 2, shape = 1)+
  geom_point(aes(y = Daniel), size = 2)+
  # geom_text(aes(y = Brian, label = round(Brian, digits = 2)),  
  #           color = "white", fontface = "bold", size = 3)+
  # geom_text(aes(y = Daniel, label = round(Daniel, digits = 2)), 
  #           color = "white", fontface = "bold", size = 3)+
