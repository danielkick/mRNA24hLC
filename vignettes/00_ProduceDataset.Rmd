...
## Simulate each cell ====
pb = txtProgressBar(min = 0, max = length(unique_abfs), 
                    initial = 0, 
                    style = 3, char = '█') 
for (i in seq_along(unique_abfs)){
  ...
  if (current_trace$process){
    ...
    for (j in seq_along(unique_cells)){
      ...
      current_cell_params <- all_cell_params[
        all_cell_params$File == current_file &
      ...
      if (nrow(current_cell_params)==1){ # if there are no acceptable parameters, current_cell_params will be nrow == 0 
        if (unlist(current_cell_params[, "Channel"]) == "In4"){
          ...
          path_obs_v_file <- here(
            "data", "predicted_voltage", paste(str_remove(current_file, ".abf"), "epsp", "In4", "actual_v.rds", sep = "-"))
          path_sim_v_file <- here(
            "data", "predicted_voltage", paste(str_remove(current_file, ".abf"), "epsp", "In4", "predict_v.rds", sep = "-"))
          
          path_plt_v_file <- here(
            "data", "data_audits", "epsp", paste(str_remove(current_file,".abf"),"epsp", "In4", "segment.pdf", sep = "-"))
          
          if(!(file.exists(path_obs_v_file) & 
               file.exists(path_sim_v_file) &
               file.exists(path_plt_v_file) )){
            ...
            out <- predict_epsp_responses(trace = current_trace,
                                          ...
                                          i_ch = "In7")
            # Save for review:
            saveRDS(current_trace[, c("In4", "In7", "Sweep", "Time")], path_obs_v_file)
            
            saveRDS(out$predictions, path_sim_v_file)
            
            save_plot(path_plt_v_file, out$diagnostic_plots$trace, base_height = 7, base_width = 10)
          }
        } else if (unlist(current_cell_params[, "Channel"]) == "In9"){
          
          path_obs_v_file <- here(
            "data", "predicted_voltage", paste(str_remove(current_file, ".abf"), "epsp", "In9", "actual_v.rds", sep = "-"))
          path_sim_v_file <- here(
            "data", "predicted_voltage", paste(str_remove(current_file, ".abf"), "epsp", "In9", "predict_v.rds", sep = "-"))
          
          path_plt_v_file <- here(
            "data", "data_audits", "epsp", paste(str_remove(current_file,".abf"),"epsp", "In9", "segment.pdf", sep = "-"))
          ...
          if(!(file.exists(path_obs_v_file) & 
            ...
            # Save for review:
            saveRDS(current_trace[, c("In9", "In12", "Sweep", "Time")], path_obs_v_file)
...
pb = txtProgressBar(min = 0, max = length(brian2_trace_sets), 
...
epsp_summary_list <- map(seq(1, length(brian2_trace_sets)), function(i){
  ...
  temp <- full_join(temp_a, select(temp_p, -Time), by = c("Sweep", "nrow") ) %>% select(-nrow)
...
pb = txtProgressBar(min = 0, max = length(iter.over), 
                    initial = 0, style = 3, char = '█') 
