...
current_file <- as.character(abfs_one_exp[i, "File"])
current_ABFType <- as.character(abfs_one_exp[i, "ABFType"])
...
# Does it exist?
if(!file.exists(paste0(abf_dir_path, current_file))){
  # write out couldn't find this, doesn't exist, into log
  warning(paste(current_file, "was not found in the specified location.\nRefer to /data/data_audits/RejectedABFs.csv"))
  
  FileRejectionReport <- data.frame(
    FilePath = abf_dir_path,
  ...
  temp_trace_size <- gdata::humanReadable(file.size(paste0(abf_dir_path, current_file)))
  temp_trace <- prep_abf(Path = abf_dir_path, FileName = current_file)
  ...
  if ((expected_size & expected_sweeps) | (current_file %in% force_process_ABF)){
    process_current_ABF <- T
  } else {
    # TODO If there was a problem write out to a log in data/data_audits/    
    
    warning(paste(current_file, "was rejected. \nAdd it to `force_process_ABF` if you're sure it should be included. \nRefer to /data/data_audits/RejectedABFs.csv"))
...
# Get filesize
abfs_to_process$FileSize <- 0

for (i in seq(1, nrow(abfs_to_process))){
  temp_path <- paste0(abf_dir_path, as.character(abfs_to_process[i, "File"]))
  
  if (file.exists(temp_path)){
    abfs_to_process[i, "FileSize"] <- file.size(temp_path) 
  }
}

# Setting heruritcs for the right file sizes

# # Examine File Sizes:
# abfs_to_process %>% 
#   mutate(FileSize_h = gdata::humanReadable(FileSize)) %>% 
#   group_by(Type, FileSize_h) %>% 
#   tally() %>% 
#   ungroup() %>% 
#   group_by(Type, FileSize_h)
...
abfs_to_process <- abfs_to_process %>% 
  mutate(FileSize_h = gdata::humanReadable(FileSize)) %>% 
  mutate(ExpectedSize = case_when(ABFType == "epsp" & FileSize_h == " 3.8 MiB" ~ T,
                                  ABFType == "fi" & FileSize_h == "45.8 MiB" ~ T,
                                  ABFType == "gjcc" & FileSize_h == "14.3 MiB" ~ T,
                                  ABFType == "gjvc" & FileSize_h == " 5.5 MiB" ~ T
                                  ))

# see those not of expected size
# abfs_to_process[is.na(abfs_to_process$ExpectedSize), ]
...
# start with any except for fi/epsp
...
#TODO get all the non 0 size files and order them based on type so that the epsp/fi are last. 
#TODO we will check that there are tau/rin for epsp/fi and not run them if we don't have that information.

i=1



temp_trace <- prep_abf(Path = abf_dir_path, 
                       FileName = as.character(abfs_to_process[i, "File"])
                       
                       # FileName = "190903_0025.abf" # fi
                       # FileName = "190903_0023.abf" # epsp
                       # FileName = "190903_0021.abf" # gjcc
                       # FileName = "190903_0020.abf" # gjvc
                       
                       )

ABFType <- as.character(abfs_to_process[i, "ABFType"])

# Expected size?
ExpectedSize <- as.logical(abfs_to_process[i, "ExpectedSize"])

# Contains expected number of sweeps?
if (ABFType == "fi"){
  ExpectedSweeps <- length(unique(temp_trace$Sweep))==11
  require_rin_tau <- F
} else if(ABFType == "epsp"){
  ExpectedSweeps <- length(unique(temp_trace$Sweep))==4
  require_rin_tau <- F
} else if(ABFType == "gjcc"){
  ExpectedSweeps <- length(unique(temp_trace$Sweep))==5
  require_rin_tau <- T
} else if(ABFType == "gjvc"){
  ExpectedSweeps <- length(unique(temp_trace$Sweep))==9
  require_rin_tau <- T
}

#TODO which files should be named here?
ForceAllowFiles <- c()
# if file is one of those named in "ForceAllowFiles" run it even if it doesn't meet the criteria.


## Check if we should process a file and do so. ====

if ( !((ExpectedSize & ExpectedSweeps) | (as.character(abfs_to_process[i, "File"]) %in% ForceAllowFiles)) ){
  # Fail state
  warning(paste(as.character(abfs_to_process[i, "File"]), "was rejected. Add it to `ForceAllowFiles` if you're sure it should be included."))
  # write out a doc with those files not run named and why they were not run. Put this in data/data_audits
  ...
  } else if (require_rin_tau){
    #TODO if epsp/fi access correct rin/tau
    if (ABFType == "fi"){
      #TODO
