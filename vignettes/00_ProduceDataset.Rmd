...
# walk(seq_along(unique_abfs), function(i){
for (i in seq_along(unique_abfs)){
  ...
  if (current_trace$process){
    ...
    # added to deal with this error:
    # Error in lm.fit(x, y, offset = offset, singular.ok = singular.ok, ...) : 
    #  0 (non-NA) cases 
    
    # out <- tryCatch({process_gjvc_trace(trace = current_trace)}, 
    #          error = function(e) {
    #            return(list(error_msg = e))
    #          })
    
    out <- process_gjvc_trace(trace = current_trace)
    
    if (exists("out$error_msg")){
      
      # # If there was an error write it out here
