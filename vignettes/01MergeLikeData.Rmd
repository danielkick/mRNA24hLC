...
SwapNames <- mRNAInfo[tolower(mRNAInfo$InData) %in% ToCorrect, c("InData", "RGeneName")]
SwapNames$InData <- tolower(SwapNames$InData)

for (i in 1:nrow(SwapNames)){
  ToCorrect[ToCorrect == as.character(SwapNames[i, "InData"])] <- as.character(SwapNames[i, "RGeneName"])  
}
