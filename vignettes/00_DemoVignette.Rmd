...
#         here("data", "data_audits", paste(str_remove(input_file, ".abf"), "fi", "predict_v.rds", sep = "-")))
# 
...
M <- readABF_as_matrix2(path = 
                          # paste0(abf_dir_path, "190915_0019.abf"),
                          # paste0(abf_dir_path, "181219_0081.abf"),
                        # paste0(abf_dir_path, "190122_0021.abf"),
                        paste0(abf_dir_path, "190206_0053.abf"),
                   channels = "all",
                   relative.time = T)
...
temp_In9_Tau <- abs(mean(unlist(out$df[, "In9_Tau_est"])))
temp_In9_Rin <- abs(mean(unlist(out$df[, "In9_R1"])))
...
i=1
temp <- temp[temp$Sweep == i, ]


place_data_for_predict_voltage_trace(input.df = temp,
                                     Time.ch = "Time", 
                                     Inj.ch = "In12")
...
#TODO how do we calculate a good offset? Is the trace I'm working with representatiave?

temp_v <- cbind(temp,
                predict = predicted_response[, "predicted"])

# Get the voltage difference from the end where there is a constant injection
temp_v_summary <- temp_v %>% ungroup() %>% filter(Time >= 19.38) %>% mutate(temp = predict - In9) %>% summarise(mean = mean(temp),
                                                                                          median = median(temp),
                                                                                          sd = sd(temp))


# temp_v$abs_inj <- abs(temp_v$In12)
# temp_v[temp_v$abs_inj <= quantile(temp_v$abs_inj, 0.1), ] %>% mutate(temp = predict - In9) %>% summarise(mean = mean(temp),
#                                                                                                          median = median(temp),
#                                                                                                          sd = sd(temp))

ggplot(temp_v, aes(Time))+
  geom_path(aes(y = In9))+
  geom_path(aes(y = predict - unlist(temp_v_summary[, "median"])), color = "cornflowerblue")+
  geom_path(aes(y = In12), color = "firebrick")
  # geom_vline(xintercept = 19.38)+
  # coord_cartesian(x = c(19.36, 19.4)) #19.68685))

# ggplot(downsample_data(temp_v, len = 5000), aes(In12))+
#   geom_point(aes(y = In9))+
#   geom_point(aes(y = predict), color = "cornflowerblue")
# 
# ggplot(downsample_data(temp_v, len = 5000), aes(In9, y = predict))+
#   # geom_point(aes(y = In9))+
#   geom_abline(intercept = seq(50, 100, by = 25), slope = 1)+
#   geom_smooth()+
...
# take the lowest min(abs()) x percent of i inj; find voltage differnce between them, subtract


# predicted_response_list <- map(1:4, function(i){
#   place_data_for_predict_voltage_trace(input.df = temp[temp$Sweep == i, ],
#                                        Time.ch = "Time", 
#                                        Inj.ch = "In12")
#   
...
#   function(i){
#     ggplot()+
#       geom_line(data = predicted_response_list[[i]], aes(Time, predicted-56.5), color = "red")+
...
```{r}
# function(trace = trace_list$GJCC){

# temp <- trace_list$EPSP
#TODO clean up

# tau <- abs(mean(unlist(out$df[, "In9_Tau_est"])))
# rin <- abs(mean(unlist(out$df[, "In9_R1"])))
# 
# v_ch = "In9"
# i_ch = "In12"
